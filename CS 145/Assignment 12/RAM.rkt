...
;; (ram-store ram addr w) produces ram with word at address addr replaced by w
;;     addr and w must be non-negative integers
;; (ram-fetch ram addr) produces the value previously stored at address addr in ram

;; examples:
;; (define x (ram-store (ram-store ram 1 1) 2 0));  store 1 at address 1, 0 at address 2
...
(define (ram-fetch a b)
  ...
  (if (number? r) r r)); (error "attempt to fetch undefined value at " a)))
