...
(define (difference s1 s2)
  (cond
    ...
    [(> (size s1) (size s2))
      (cond
        [(empty? s2) s1]
        [(equal? (size s1) (size (insertavl s1 (node-key s2)))) (difference (deleteavl s1 (node-key s2)) (deleteavl s2 (node-key s2)))]
        [true (difference s1 (deleteavl s2 (node-key s2)))])]
    [true (difference-help s1 s2 empty)]))

(define (difference-help s1 s2 s3)
  (cond
    [(empty? s1) s3]
    [(equal? (size s2) (size (insertavl s2 (node-key s1)))) (difference-help (deleteavl s1 (node-key s1)) (deleteavl s2 (node-key s1)) s3)]
