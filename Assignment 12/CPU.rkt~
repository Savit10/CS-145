...
(define (cpu-add ram target-addr source-addr)            ; [target-addr] = [target-addr]+[source-addr]
  (ram-store ram target-addr (+ (ram-fetch ram target-addr) (ram-fetch ram source-addr))))

(define (cpu-sub ram target-addr source-addr)            ; [target-addr] = max([target-addr]-[source-addr], 0)
  (ram-store ram target-addr (max (- (ram-fetch ram target-addr) (ram-fetch ram source-addr)) 0)))
...
(define (cpu-fetch ram target-addr source-ptr-addr)      ; [target-addr] = [[source-ptr-addr]]
  (cpu-copy ram target-addr (ram-fetch ram source-ptr-addr)))

(define (cpu-store ram target-ptr-addr source-addr)      ; [[target-ptr-addr]] = [source-addr]
...
(define (ram-load ram load-addr lst) ; [target-addr] = (car lst), [target-addr+1] = (cadr lst), ...
  (cond
    [(empty? lst) ram]
    [true
     (ram-load (ram-store ram load-addr (car lst))
               (add1 load-addr) (cdr lst))]))
